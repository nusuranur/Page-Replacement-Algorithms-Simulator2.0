function saveSimulationData(algorithm, refArray, hits, faults) {
    ...
    // ✅ Fix: convert array to space-separated string
    const referenceString = refArray.join(" ");

    // ✅ Build parameters
    const params = `algorithm=${algorithm}&reference_string=${encodeURIComponent(referenceString)}&frames=${encodeURIComponent(document.getElementById("frames").value)}&hits=${hits}&faults=${faults}&hit_ratio=${hitRatio}&miss_ratio=${faultRatio}`;

    console.log("🟢 Sending to server:", params); // ✅ Check in console

    xhr.onreadystatechange = function () {
        if (xhr.readyState === 4) {
            console.log("🟢 Server response:", xhr.responseText); // ✅ Output server reply
        }
    };
...
function FIFO() {
  ...
  algorithm,
  ref_string: refString.join(" "),
  hits,
  faults,
  hit_ratio: (hits / (hits + faults)).toFixed(2),
  fault_ratio: (faults / (hits + faults)).toFixed(2)
});


    // ✅ Call your data saving function here:
    saveSimulationData(
        "FIFO",              // Algorithm name
        ref,                 // Reference string array
        obj.pageHits,        // Total hits
        obj.pageFaults       // Total faults
    );
}
